...
def train_model(X_train, y_train):
    ...
    """
    # Set up model with specified parameters
...
def save_model(model, path):
    ...
    """
    # Open file in write mode and save model inside file
...
def performance_on_categorical_slice(
    ...
    Inputs
    ------
    data : pd.DataFrame (Dataframe containing the features and label. Columns in `categorical_features`)
    column_name : str (Column containing the sliced feature)
    slice_value : str, int, float (Value of the slice feature)
    categorical_features: list (List containing the names of the categorical features (default=[]))
    ...
    model : model (Model used for the task)
    ...
    X_slice, y_slice, _, _ = process_data(
        X = data[data[column_name] == slice_value], # filter dataframe
        categorical_features = categorical_features,
        label = label,
        training = False, # leave on FALSE to not refit
        encoder = encoder,
        lb = lb 
    )
    
    preds = inference(model, X_slice) # prediction on X_slice
